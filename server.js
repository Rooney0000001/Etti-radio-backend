...
async function fetchHeadlines() {
  ...
  for (const url of queries) {
    try {
      ...
      if (!res.ok) {
        console.error('GNews returned an error. Status:', res.status, 'Body:', JSON.stringify(data));
        continue;
      }

      if (data.articles) {
        console.log('GNews returned', data.articles.length, 'articles for query:', url.split('?')[1].split('&apikey')[0]);
        articles = articles.concat(data.articles);
      } else {
        console.log('GNews response had no articles field:', JSON.stringify(data));
      }
    } catch (err) {
      console.error('Error fetching headlines:', err.message);
    }
  }
  console.log('Total articles collected this cycle:', articles.length);
  return articles;
}
...
Description: ${article.description || ''}`;
  ...
  try {
    ...
    if (!res.ok) {
      console.error('Gemini returned an error. Status:', res.status, 'Body:', JSON.stringify(data));
      return null;
    }

    const text = data.candidates?.[0]?.content?.parts?.[0]?.text;
    if (!text) {
      console.log('Gemini response had no usable text:', JSON.stringify(data));
    }
